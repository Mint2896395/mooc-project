  ...
  return (
    ...
    {!item.children ? (
      <li className='border-b border-gray-200'>
        <Link href={item.path} className={`${router.pathname == item.path ? "font-normal text-[var(--global-color-primary)] border-l-4 border-orange-300" : "font-light text-gray-500"} font-[Noto_Sans_Thai] flex items-center justify-between w-full py-5 text-left  pl-5 pr-2`} >
          {item.title}
        </Link>
      </li>
    ) : (
      <li className={`#accordion-flush-heading-${item.id}`}>
        <button type="button" className={`${router.pathname == item.path ? "font-normal text-[var(--global-color-primary)] border-l-4 border-orange-300 border-b-gray-200" : "font-light text-gray-500"} font-[Noto_Sans_Thai] flex items-center justify-between w-full py-5 text-left pl-5 pr-2 border-b border-gray-200`} data-accordion-target={`#accordion-flush-body-${item.id}`} aria-expanded="false" aria-controls={`#accordion-flush-body-${item.id}`}>
          {item.title}
